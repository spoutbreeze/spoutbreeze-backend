...
class ChannelsServiceCached(ChannelsService):
    ...
    async def get_channel_by_id(self, db: AsyncSession, channel_id: UUID) -> Optional[ChannelResponse]:
        return await super().get_channel_by_id(db, channel_id)

    @cached_db(ttl=settings.cache_ttl_medium, key_prefix="channels_by_name")
    async def get_channel_by_name(self, db: AsyncSession, channel_name: str, user_id: UUID):
    ...
    async def get_channel_recordings(self, db: AsyncSession, channel_id: UUID, user_id: UUID) -> Dict[str, Any]:
        return await super().get_channel_recordings(db, channel_id, user_id)

    # WRITES (invalidate)
    async def create_channel(self, db: AsyncSession, channel_create: ChannelCreate, user_id: UUID) -> ChannelResponse:
        res = await super().create_channel(db, channel_create, user_id)
        await self._invalidate_after_change()
        return res

    async def update_channel(self, db: AsyncSession, channel_id: UUID, channel_update: ChannelUpdate, user_id: UUID) -> Optional[ChannelResponse]:
        res = await super().update_channel(db, channel_id, channel_update, user_id)
        await self._invalidate_after_change()
        return res

    async def delete_channel(self, db: AsyncSession, channel_id: UUID, user_id: UUID) -> bool:
