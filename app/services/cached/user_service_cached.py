...
class UserServiceCached:
    ...
    @cached_db(ttl=900, key_prefix="user_profile")  # 15 minutes
    async def get_user_by_id_cached(self, user_id: UUID, db: AsyncSession) -> Optional[User]:
        """Get user by ID with caching"""
        stmt = select(User).where(User.id == user_id)
        res = await db.execute(stmt)
        return res.scalars().first()

    @cached_db(ttl=900, key_prefix="user_keycloak")  # 15 minutes
    async def get_user_by_keycloak_id_cached(self, keycloak_id: str, db: AsyncSession) -> Optional[User]:
        """Get user by Keycloak ID with caching"""
        stmt = select(User).where(User.keycloak_id == keycloak_id)
        res = await db.execute(stmt)
        return res.scalars().first()

    @cached_db(ttl=1800, key_prefix="user_roles")  # 30 minutes - roles change less frequently
    ...
    @cached_db(ttl=1800, key_prefix="users_list")  # 30 minutes for admin lists
