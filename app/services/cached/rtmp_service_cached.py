...
class RtmpEndpointServiceCached(RtmpEndpointService):
    ...
    async def create_rtmp_endpoints(self, rtmp_endpoints: CreateRtmpEndpointCreate, user_id: UUID, db: AsyncSession) -> RtmpEndpointResponse:
        res = await super().create_rtmp_endpoints(rtmp_endpoints, user_id, db)
        await self._invalidate_after_change(user_id=user_id)
        return res

    async def update_rtmp_endpoints(self, rtmp_endpoints_id: UUID, rtmp_endpoints_update: RtmpEndpointUpdate, db: AsyncSession) -> Optional[RtmpEndpointResponse]:
    ...
    async def delete_rtmp_endpoints(self, rtmp_endpoints_id: UUID, user_id: UUID, db: AsyncSession) -> Optional[RtmpEndpointDeleteResponse]:
